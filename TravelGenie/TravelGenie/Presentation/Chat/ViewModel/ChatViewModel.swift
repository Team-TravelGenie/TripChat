...
private extension ChatViewModel {
    ...
    enum Constant {
        ...
        static let openAISystemPrompt: String = """
            ...
            step 3. 선별한 여행지 정보를 JSON 형식으로 반환합니다. JSON 응답의 구조는 다음과 같습니다. recommendationItems라는 배열 안에 각각의 여행지 정보를 포함합니다. 각 여행지 정보에는 "country" (한국어 국가 이름), "spotKorean" (여행지 이름 국문), "spotEnglish" (여행지 이름 영문)을 할당합니다.
            ...
            step 4. 이후 사용자가 추가로 여행지 추천을 요청하는 경우, 최초 제공한 여행지 정보 이외의 여행지 정보를 JSON 형식으로 반환합니다. 그러나 사용자가 추천 이외의 질문을 하는 경우, 텍스트 형식으로 답변을 제공합니다. 이때 여행지 관련 정보는 출처가 분명하며 사실에 기반한 것이어야 합니다.
            """
    }
}
